...
class register:
  ...
  def execute(self, data, body, conn):
    ...
    XML = "<profile><info><user_id>" + data[0] + "</user_id><first>" + data[1] + "</first><last>" + data[2] + "</last><ip>" + conn.getpeername()[0] + "</ip><public_key>" + data[3] + "</public_key><updated>" + datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M:%S') + "</updated></info></profile>"

    try:
      User(user_id=data[0], first=data[1], last=data[2], profile=XML, public_key=data[3]).save()
    except:
