...
def runServer(s):
  while True:
    ...
    try:
      ...
      value = rsa_key.decrypt(raw_cipher_data)
      public_key_file = open("UserPublicKey", 'wb')
      public_key_file.write("-----BEGIN PUBLIC KEY-----\n" + user.public_key + "\n-----END PUBLIC KEY-----\n")
      public_key_file.close()
      public_key_file = open("UserPublicKey", 'r')
      user_key = RSA.importKey(public_key_file.read())
      public_key_file.close()
      decrypt = user_key.decrypt(value)
    except:
      decrypt = value
